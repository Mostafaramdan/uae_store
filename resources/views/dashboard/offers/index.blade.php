...
@section ('content')
    ...
    <div  id="alert">
        ...
        <div class="d-flex align-items-center mb-4">
          <h2 class="m-0 list-lang" ar='# العروض' en=' # offers'></h2>
        </div>
        ...
        <form class="mb-4" id="getOptions">
            ...
            <div class="m-2">
              <select class="custom-select" name="sortBy">
                ...
                <option value="id">تاريخ الانشاء</option>
              </select>
            </div>
        ...
        <div class="flex-grow-1"></div>
          ...
          <div class="table-responsive">
            <table class="table bg-light mb-4 tableInfo" id="tableInfo" dir="rtl">
              @include('dashboard.offers.tableInfo')
            </table>
        </div>
        <!-- pagination -->
        <div class="paging">  
      ...
      <!-- Large modal -->
      @include('dashboard.offers.viewModal')
      @include('dashboard.offers.addEditModal')
</div>
@push('script')
<script>
  ...
  $("body").on("click",".edit",function(){
    ...
    $.get( "{{Request::segment(2)}}/getRecord/"+id,function( record ) {
      ...
      if(record.categories_id){
        $("select[name='action'] option[value='categories']").prop('selected', true);;
        $(".action").find(".form-group").addClass("d-none").parents(".action").find(".categories").removeClass("d-none").siblings().find("select option:first").prop('selected', true);

      }else if (record.regions_id){
        $("select[name='action'] option[value='regions']").prop('selected', true);;
        $(".action").find(".form-group").addClass("d-none").parents(".action").find(".regions").removeClass("d-none").siblings().find("select option:first").prop('selected', true);
      }else if (record.products_id){
        $("select[name='action'] option[value='products']").prop('selected', true);
        $(".action").find(".form-group").addClass("d-none").parents(".action").find(".products").removeClass("d-none").siblings().find("select option:first").prop('selected', true);
