...
public static function index()
{
    ...
    return view('dashboard.contacts.index',compact("records","totalPages",'currentPage'));
}   
...
public static function indexPageing(Request $request)
{
    ...
    if($request->search){
        ...
        $records= $records->filter(function($item) use ($search) {
                if( stripos($item['users']['name'],$search)!== false || stripos($item['users']['phone'],$search)!== false || stripos($item['users']['email'],$search)!== false  || stripos($item['message'],$search)!== false )
                    return true;
                else 
                    return false;
            });
    ...
    $tableInfo= (string) view('dashboard.contacts.tableInfo',compact('records'));
